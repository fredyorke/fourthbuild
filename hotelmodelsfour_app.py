...
if uploaded_file:
    ...
    # Check for date and numeric columns
    date_cols = df.select_dtypes(include=['object', 'datetime']).columns.tolist()
    numeric_cols = df.select_dtypes(include=['float64', 'int64']).columns.tolist()

    if not date_cols or not numeric_cols:
        st.error("The uploaded CSV must contain at least one date column and one numeric column.")
    else:
        ...
        if model_choice == "Holt-Winters":
            try:
                ...
                # Evaluation
                if len(ts) >= 12:  # Ensure there is enough data for evaluation
                    train = ts[:-12]
                    test = ts[-12:]
            ...
            try:
                ...
                if len(prophet_df) >= 12:  # Ensure there is enough data for evaluation
